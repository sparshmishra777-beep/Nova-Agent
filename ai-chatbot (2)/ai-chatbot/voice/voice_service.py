...
def process_voice_input(audio_bytes: bytes, session_id: str, user_name: str = None) -> dict:
    """
    ...
    # Resolve name from parameter or memory
    resolved_name = user_name or memory_manager.get_user_name(session_id, "Guest")

    # 2. Query the main orchestrator with user name context
    result = orchestrate_message(user_text, session_id, user_name=resolved_name)
