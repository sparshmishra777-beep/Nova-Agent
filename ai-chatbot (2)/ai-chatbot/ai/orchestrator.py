...
def orchestrate_message(user_message: str, session_id: str, user_name: str = None) -> dict:
    """
    Main Orchestrator loop. Routes, acts on tools, queries database, formats, and updates memory.
    Supports user identity greetings and session memory state.
    """
    # 1. Update user identity if provided
    if user_name:
        memory_manager.set_user_name(session_id, user_name)
    
    resolved_user_name = memory_manager.get_user_name(session_id, "Guest")
    
    # 2. Check if this is the first exchange in history
    history = memory_manager.get_history(session_id)
    is_first_query = len(history) == 0
    ...
    if intent == "voice_control":
        ...
        if is_first_query:
            response_text = f"Hello {resolved_user_name}! " + response_text
        ...
        try:
            ...
            if execution_res["status"] == "success":
                ...
                if is_first_query:
                    verbal_summary = f"Hello {resolved_user_name}! " + verbal_summary
                ...
                if is_first_query:
                    error_response = f"Hello {resolved_user_name}! " + error_response
        ...
        system_instruction = (
            f"You are an expert Enterprise Voice AI Assistant. You help users with business analytics. "
            f"The user's name is {resolved_user_name}. Always refer to them by name if greeting them or when appropriate. "
        ...
        try:
            ...
            # If the model didn't prefix the greeting itself on the first message, we can enforce it
            if is_first_query and not re.search(rf"\bhello\b|\bhi\b", chat_response, re.IGNORECASE):
                chat_response = f"Hello {resolved_user_name}! " + chat_response
...
if __name__ == "__main__":
    ...
    print("Sending greeting as Manu:")
    r1 = orchestrate_message("Hello there!", test_session, "Manu")
    print("Response:\n", r1["response"])
    print("-" * 40)
