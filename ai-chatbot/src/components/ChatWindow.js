...
export default function ChatWindow() {
  ...
  useEffect(() => {
    ...
    if (SpeechRecognition) {
      ...
      recognition.onstart = () => {
        setIsListening(true);
        stopAudio(); // Interrupted playback when user starts talking
      };
      ...
      recognition.onresult = (event) => {
        ...
        const newText = baseText + (baseText && (finalTranscript || interimTranscript) ? ' ' : '') + finalTranscript + interimTranscript;
        setInput(newText);
      };
  ...
  // Play audio response (TTS playback)
  ...
  const toggleListening = () => {
    ...
    if (isListening) {
      ...
      setError(null);
      stopAudio(); // Interrupt audio immediately
  ...
  const handleSend = async () => {
    ...
    stopAudio(); // Interrupt any playing audio before sending new query
