...
def validate_sql(sql_query: str) -> tuple[bool, str]:
    ...
    # Scan raw string for forbidden keywords using Regex for security
    lower_query = cleaned.lower()
    for kw in FORBIDDEN_KEYWORDS:
        import re
        if re.search(r'\b' + kw + r'\b', lower_query):
            return False, f"Unsafe SQL detected: Keyword '{kw}' is forbidden."
